...
const Searchbar: React.FC<SearchbarProps> = ({ onChange }) => {
  ...
  <form action="" className="relative">
    <input 
      type="text" 
      placeholder="Search" 
      className={`md:w-[500px] w-full sm:w-80 py-2 px-2 rounded-full bg-transparent border ${isFocused ? "border-custom-color3 text-custom-color3" : "border-custom-color3"} focus:outline-none`}
      onFocus={handleFocus} 
      onBlur={handleBlur}
