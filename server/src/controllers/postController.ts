...
export const deletePost = (req: Request,
    ...
    next: NextFunction) => {
        try {
            ...
            jwt.verify(token, process.env.ACCESS_TOKEN_SECRET || '', async (error: VerifyErrors | null, userInfo: any) => {
              if (error || !userInfo) return res.status(403).json("Token is not valid");
